...
class ExpertCluster:
    ...
    def delegate_batch(self, batch: dict) -> List[int]:
        """
        For each sample in the batch, choose the expert with softmax sampling
        over normalized per-sample perplexities.
        Returns a list of expert IDs of length batch_size.
        """
        all_perplexities = []  # List[Tensor] of shape (batch_size,)
        for expert in self.experts:
            ppl = expert.compute_perplexity(batch)  # List[float] of shape (batch_size,)
            all_perplexities.append(torch.tensor(ppl, device=self.device))  # ensure on same device
    ...
    should incorporate some regularization (randomness). As per Daniel's suggestion.
        ...
        self.selection_temperature = max(0.001,self.selection_temperature * 0.9995)  # decay to 0.001 at end of epoch 1
