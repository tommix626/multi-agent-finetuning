...
class ExpertAdapter:
    def __init__(self, base_model: PeftAdapterMixin, config:PeftConfig, name: str, tokenizer, device="cuda", target_modules=None):
        self.device = device
    ...
    def attach_to_base_model(self, lora_config: LoraConfig):
        """
        Attach a itself (a LoRA adapter) to the base model for training on this example.
        """
        self.base_model.add_adapter(lora_config, self.name)
    ...
    def load_adapter(self, adapter_path: Optional[str]=None):
        ...
        self.base_model.load_adapter(adapter_path, self.name)
    ...
    def unload_to_base_model(self):
        ...
        """
        curr_active_adapters = self.base_model.active_adapters()
    ...
    def save_adapter(self, ):
        ...
        self.base_model.save_pretrained(save_path, adapter_name=self.name)
